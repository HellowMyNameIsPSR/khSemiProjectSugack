...
/**
 * Servlet implementation class InsertReApplyFormServlet
 */
@WebServlet("/insertReApply.at")
public class InsertReApplyFormServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertReApplyFormServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			int result = new AuthorService().insertReApplyFile(memberId, fileList);
			
			if(result > 0) {
				response.sendRedirect("views/application/checkApply.jsp");
			} else {
				System.out.println("1차 입점 재신청 성공");
				for(int i = 0; i < changeName.size(); i++) {
					File failedFile = new File(filePath + changeName.get(i));
					System.out.println(failedFile.delete());
				} //end for
				request.setAttribute("msg", "1차 입점 재신청 성공");
